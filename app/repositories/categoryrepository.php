...
class CategoryRepository extends Repository
{
    ...
    {
        try {
            $stmt = $this->connection->prepare("INSERT into category (eventType) VALUES (?)");

            $stmt->execute([$category->eventType]);
    ...
    {
        try {
            $stmt = $this->connection->prepare("UPDATE category SET eventType = ? WHERE id = ?");

            $stmt->execute([$category->eventType, $id]);
